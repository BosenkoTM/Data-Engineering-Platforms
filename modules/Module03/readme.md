...
1. Реализовать дашборд согласно [варианта](https://github.com/BosenkoTM/Data-Engineering-Platforms/blob/master/modules/Module03/Lab%203.1/01ex.md) на основе [Визуализация данных из CSV-файла](https://cloud.yandex.ru/docs/datalens/tutorials/data-from-csv-visualization).
...
2. Проанализировать дашборды на работе или, ранее выполненных, лабораторных работ 1-2. Улучшить их на основе [правил дизайна дошбордов](https://leftjoin.ru/all/10-rules-for-better-dashboard-design/).
...
3. Установить  `Tableau Desktop` [инструкция](https://github.com/BosenkoTM/Data-Engineering-Platforms/blob/master/how-to/how-to-tabelau-desktop.md#how-to-%D0%BA%D0%B0%D0%BA-%D1%83%D1%81%D1%82%D0%B0%D0%BD%D0%BE%D0%B2%D0%B8%D1%82%D1%8C-tableau-desktop).
4. Подключитесь к БД `Postgres` и нарисуйте дашборд, который реализовали в лабораторной работе 1 или 2. Понять, как работает:
...
5. Подключитесь к БД `Postgres` и нарисуйте дашборд, который реализовали в лабораторной работе 1 или 2. Провести анализ компоненторв из предыдущего пункта и привести таблицу соответствия с [Yandex Data Lens](https://cloud.yandex.ru/services/datalens).

Вы можете просто разобрать на части дашборд из примера и создать свой на этой базе. Главное понять принципы работы.

6. Создать дашборд и создать аккаунт на [Tableau Public](https://public.tableau.com/s/) используйте VPN, куда сможете загрузить свои дашборды. Постарайтесь использовать как можно больше Tableau функциональности, но не забывайте о красоте и простоте.
...
Необходимоскачать и установить Tableau Сервер. Необходимо минимум 8 Гб оперативки (если ставить на Windows без виртуальной машины) и 16 Гб (если использовать виртуальную машину). 

1. Скачать Tableau Server [отсюда](https://www.tableau.com/support/releases/server). Можно версию 2018.1 если ограничения по оперативной памяти. Более свежая версия требует минимум 16 Гб.
2. Установить Tableau Server на Windows (можно использовать Windows 10 или Windows Server, главное 64x бит)
3. После установки заходите в него через браузер.
4. Если используете виртуальную машину, то необходимо настроить сетевой адаптер виртуальной машины, чтобы машины могли между собой соединяться [PDF файл](https://github.com/BosenkoTM/Data-Engineering-Platforms/blob/master/modules/Module03/Lab%203.1/Vmware%20network%20example.pdf), про который рассказывалось в видео [Знакомство с Tableau Server](https://youtu.be/TYEQir1seqI) [Демонстрация и Установка Tableau Server на виртуальную Машину Windows](https://youtu.be/TYEQir1seqI?t=1218).
5. Используя свой дашборд, который  построили в Tableau Desktop на базе Postgres, загрузите его в Tableau Server 2 раза. 1й раз как `Live Connection`, 2ой раз как `Tableau Extract`. Попробуйте понять в чем разница.
6. Установите tabcmd там же, где и клиент Tableau и попробуйте обновить экстракт и скачать PDF файл дашборда.
7. Попробуйте использовать [TabPy](https://github.com/tableau/TabPy) для Tableau Desktop/Server.
...
1. Скачать Power BI дашборд и файл Excel с данными и постарайтесь выполнить задание.
2. Задание [здесь](https://github.com/Data-Learn/data-engineering/tree/master/DE-101%20Modules/Module03/DE%20-%20101%20Lab%203.1/3_11_PowerBI).
...
1. Создать опрос на основе [материала]() . Использовать Google Forms или Survey Monkey, чтобы создать опрос, разошлите коллегам на работе (если есть такая возможность и вы работаете с данными).
2. Визуализируйте результат. Для выполнения  задания, сам опрос (или его пример) и пример визуализации опроса нужно сохранить в  Git.
...
1. Используя систему [STAR](https://hr-portal.ru/story/metodika-star-dlya-prohozhdeniya-strukturirovannyh-sobesedovaniy), опишите несколько кейсов из вашего BI опыта. 
2. На сайте HH.ru сделайте себе еженедельную рассылку на интересные вам вакансии (изучайте рынок заранее).
3. Сделайте резюме в git, используя markdown как в [примере](https://elipapa.github.io/markdown-cv/) или опубликуйте в Tableau Public, как в [примере](https://public.tableau.com/en-us/s/interactive-resume-gallery).
