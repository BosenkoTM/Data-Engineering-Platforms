...
# Параметры подключения без ключа 'schema' (psycopg2 не принимает schema в DSN)
CONN_CONFIG = {k: DB_CONFIG[k] for k in ('host', 'port', 'database', 'user', 'password') if k in DB_CONFIG}
...
def connect_to_database():
    ...
    try:
        connection = psycopg2.connect(**CONN_CONFIG)
