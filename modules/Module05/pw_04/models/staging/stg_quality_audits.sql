...
SELECT
    audit_id,
    PARSE_DATE('%Y-%m-%d', audit_date) as audit_date,
    auditor_name,
    department,
    audit_type,
    process_name,
    ...
    audit_status,
    PARSE_DATE('%Y-%m-%d', next_audit_date) as next_audit_date,
    ...
    DATE_DIFF(PARSE_DATE('%Y-%m-%d', next_audit_date), PARSE_DATE('%Y-%m-%d', audit_date), DAY) as days_until_next_audit
